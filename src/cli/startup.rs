...
fn spawn_background_update_check(args: &Args) {
    ...
    // Stage 2C (chore/ssc-tui-baseline): the SAITEC-backend update push
    // channel is retired. Selfdev / non-release builds keep their git-pull
    // hint via spawn_hot_exec_update_check, but there is no published
    // /api/v1/tui/* server-side check to bridge to a client.

...
fn ensure_external_api_key_auth_allowed_for_explicit_choice(env_key: &str) -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external {} credentials. Run `{}` to authenticate with SSC-TUI directly.",
        provider_name,
        login_hint
    )
}
...
fn ensure_gemini_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting Gemini CLI credentials. Run `jcode login --provider gemini` to authenticate with SSC-TUI directly."
    )
}
...
fn ensure_copilot_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Copilot credentials. Run `jcode login --provider copilot` to authenticate with SSC-TUI directly."
    )
}
...
fn ensure_cursor_auth_allowed_for_explicit_choice() -> Result<()> {
    ...
    anyhow::bail!(
        "Skipped trusting external Cursor credentials. Run `jcode login --provider cursor` to authenticate with SSC-TUI directly."
    )
}

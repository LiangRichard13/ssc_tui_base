...
pub async fn run_restart_save_command(auto_restore: bool) -> Result<()> {
    ...
    if snapshot.sessions.is_empty() {
        ...
        if auto_restore {
            println!("Automatic restore is armed for the next plain SSC-TUI launch.");
        }
        println!("\nNo active SSC-TUI windows were detected.");
        return Ok(());
    }
...
pub async fn maybe_run_pending_restart_restore_on_startup() -> Result<bool> {
    ...
    if snapshot.auto_restore_on_next_start {
        ...
        println!(
            "Found a reboot snapshot with auto-restore enabled. Restoring {} SSC-TUI window(s)...\n",
            snapshot.sessions.len()
        );
...
pub fn run_restart_restore_command() -> Result<()> {
    ...
    if launched > 0 {
        println!("Restored {} SSC-TUI window(s).", launched);
    }

...
pub(super) fn prompt_to_trust_external_auth(
    ...
    eprintln!("SSC-TUI will only read that source in place after you approve it.");
...
fn prompt_to_review_external_auth_sources(
    ...
    eprintln!();
    eprintln!("Found existing logins that SSC-TUI can reuse.");
    eprintln!("Nothing has been imported yet.");
    eprintln!(
        "Approve the sources you want SSC-TUI to import or reuse; rejected sources stay untouched."
    );
    eprintln!();

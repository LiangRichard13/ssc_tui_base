...
pub async fn login(no_browser: bool) -> Result<AntigravityTokens> {
        ...
        && let Ok(listener) = crate::auth::oauth::bind_callback_listener(DEFAULT_PORT)
    {
        ...
        if browser_opened {
            ...
            eprintln!(
                "If the browser lands on a loopback error page instead of returning to SSC-TUI, copy the full URL from the address bar and re-run with `--no-browser` to paste it manually."
            );
...
async fn manual_login(
    ...
    eprintln!(
        "After approving access, paste the full callback URL (or query string) here so SSC-TUI can verify the login state.\n"
    );
    eprintln!(

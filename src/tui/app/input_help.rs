...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "restart" => {
                "`/restart`\nRestart SSC-TUI with the current binary. Session is preserved.\nUseful after config changes, MCP server updates, or env var changes."
            }
            "rebuild" => {
                "`/rebuild`\nRun `git pull --ff-only`, `cargo build --release`, and release tests in the background. SSC-TUI stays usable and reloads automatically when the build is ready."
            }
            "selfdev" => {
                "`/selfdev`\nSpawn a new self-dev SSC-TUI session in a separate terminal.\n\n`/selfdev <prompt>`\nSpawn a new self-dev session and auto-deliver the prompt to it.\n\n`/selfdev status`\nShow current self-dev/build status."
            }
            "split" => {
            ...
            "subscription" => {
                "`/subscription`\nShow curated SSC-TUI subscription status for this session, including router config, runtime mode, curated models, and planned tier budget scaffolding."
            }
            "version" => "`/version`\nShow SSC-TUI version/build details.",

    ...
    pub(in crate::tui::app) fn build_debug_snapshot(&self) -> DebugSnapshot {
        ...
        DebugSnapshot {
            state: serde_json::json!({
                ...
                "login_picker_open": self.login_picker_overlay.is_some(),
                "account_picker_open": self.account_picker_overlay.is_some(),
                "inline_preview_open": self.inline_interactive_state.as_ref().is_some_and(|state| state.preview),
                "status_notice": self.status_notice.as_ref().map(|(value, _)| value.as_str()),

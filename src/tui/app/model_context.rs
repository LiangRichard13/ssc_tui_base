...
impl App {
    ...
    fn handle_provider_failover_prompt(&mut self, prompt: crate::provider::ProviderFailoverPrompt) {
            ...
            .cross_provider_failover
        {
            ...
            crate::config::CrossProviderFailoverMode::Countdown if !self.is_remote => {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "⚠ **{} became unavailable** — SSC-TUI will switch to **{}** in **3 seconds** unless you cancel.\n\nReason: {}\n\nRetrying would send {}. Press **Esc** to cancel.\n\n{}",
                    prompt.from_label,
                    prompt.to_label,
                    prompt.reason,
                    input_summary,
    ...
    fn format_usage_display_card(
        ...
        if refreshing {
            ...
            lines.push(
                "Use `/login` for the jcode platform login or `/account` to review provider auth, then run `/usage` again.".to_string(),
            );
            return lines.join("\n");
        } else {

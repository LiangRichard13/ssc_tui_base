...
impl App {
    ...
    fn failover_config_hint() -> &'static str {
        "To turn this off, set `[provider].cross_provider_failover = \"manual\"` in `~/.saitec_tui/config.toml` or export `JCODE_CROSS_PROVIDER_FAILOVER=manual`."
    }
    ...
    fn handle_provider_failover_prompt(&mut self, prompt: crate::provider::ProviderFailoverPrompt) {
            ...
            .cross_provider_failover
        {
            ...
            crate::config::CrossProviderFailoverMode::Countdown if !self.is_remote => {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "⚠ **{} became unavailable** — SAITEC-TUI will switch to **{}** in **3 seconds** unless you cancel.\n\nReason: {}\n\nRetrying would send {}. Press **Esc** to cancel.\n\n{}",
                    prompt.from_label,
                    prompt.to_label,
                    prompt.reason,
                    input_summary,

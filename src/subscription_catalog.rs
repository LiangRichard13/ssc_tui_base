...
mod tests {
    ...
    // Stage 2D (chore/ssc-tui-baseline): the two SAITEC session-file bridge
    // tests (configured_api_key_ignores_stored_session_when_env_bridge_missing
    // and configured_api_key_prefers_env_bridge_over_stale_session_file) are
    // removed along with src/saitec/ — there is no longer a session file to
    // bridge against. configured_api_key() itself now only consults the env
    // bridge and the env file.
}

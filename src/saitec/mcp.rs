...
///
/// **Stub (chore/ssc-tui-baseline):** baseline no longer drives SAITEC credential
/// refresh (McpConfig::load no longer injects X-API-Key). Will be removed with
/// src/saitec/ in stage 2.
#[allow(dead_code)]
pub async fn reconnect_saitec_mcp() {
    crate::logging::debug("SAITEC MCP reconnect skipped: baseline version does not drive SAITEC credential refresh");
}
...
///
/// **Stub (chore/ssc-tui-baseline):** baseline no longer drives SAITEC credential
/// refresh. Will be removed with src/saitec/ in stage 2.
#[allow(dead_code)]
pub async fn disconnect_saitec_mcp() {
    crate::logging::debug("SAITEC MCP disconnect skipped: baseline version does not drive SAITEC credential refresh");
}

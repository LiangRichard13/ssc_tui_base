...
- **Run the dev debug script after fixes** - After completing each fix, run `scripts/dev_ssc_tui.ps1` to start the TUI project so the user can inspect it.
...
## Logs
- Logs are written to `~/.ssc_tui/logs/` (daily files like `jcode-YYYY-MM-DD.log`).
...
- `~/.ssc_tui/builds/current/jcode` is the active local/source-build channel; self-dev builds and `scripts/install_release.sh` point the launcher here.
- `~/.ssc_tui/builds/stable/jcode` is the stable release channel; `scripts/install.sh` installs this and points the launcher here.
- `~/.ssc_tui/builds/versions/<version>/jcode` stores immutable binaries.
- `~/.ssc_tui/builds/canary/jcode` still exists for canary/testing flows, but it is not the primary self-dev install path.
